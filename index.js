...
function validateName(){
	...
	if (name_field.value == 0) {
		...
		error_name.innerHTML = "Nombre erroneo";
		name_field.classList.add("invalid");
		name_field.classList.add("error_icon");
		return false;
	}
...
//El campo teléfono deberá tener formato de número telefónico 


function validateTelephone(){
	var telephone_field = document.getElementById('telephone');
	var valid_telephone = /^(\+34|0034|34)?[6789]\d{8}$/;
	var error_telephone = document.getElementById('telephone_error');

	if (telephone_field.value == 0) {
		error_telephone.innerHTML = "Rellene este campo";
		telephone_field.classList.add("invalid");
		telephone_field.classList.add("error_icon");
		return false;
	}
	else if (valid_telephone.test(telephone_field.value)) {
		telephone_field.classList.remove("invalid");
		telephone_field.classList.remove("error_icon");
		error_telephone.innerHTML = "";
		telephone_field.classList.add("valid");
		telephone_field.classList.add("success_icon");
		return true;
	}
	else {
		telephone_field.classList.remove("valid");
		telephone_field.classList.remove("success_icon");
		error_telephone.innerHTML = "Número erroneo";
		telephone_field.classList.add("invalid");
		telephone_field.classList.add("error_icon");
		return false;
	}
...
function validateForm() {
	if (validateName() && validateConfirm() && validatePassword() && validateTelephone() && validateEmail() && validateName()) {
		Swal.fire(
